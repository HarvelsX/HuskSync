...
public class DataManager {
     ...
     */
    public PlayerData getPlayerDataByName(String playerName, Settings.SynchronisationCluster cluster) {
        try (Connection connection = getConnection(cluster.clusterId());
             PreparedStatement statement = connection.prepareStatement(
                     "SELECT `uuid` FROM " + cluster.playerTableName() + " WHERE `username`=? LIMIT 1;")
        ) {
            statement.setString(1, playerName);
    ...
    public PlayerData getPlayerData(Settings.SynchronisationCluster cluster, UUID playerUUID) {
        try (Connection connection = getConnection(cluster.clusterId());
             PreparedStatement statement = connection.prepareStatement(
    ...
    private void updatePlayerSQLData(PlayerData playerData, Settings.SynchronisationCluster cluster) {
        try (Connection connection = getConnection(cluster.clusterId());
             PreparedStatement statement = connection.prepareStatement(
                     "UPDATE " + cluster.dataTableName() +
                             " SET `version_uuid`=?, `timestamp`=?, `inventory`=?," +
                             "`ender_chest`=?, `health`=?, `max_health`=?, `health_scale`=?," +
                             "`hunger`=?, `saturation`=?, `saturation_exhaustion`=?, `selected_slot`=?," +
                             "`status_effects`=?, `total_experience`=?, `exp_level`=?, `exp_progress`=?," +
                             "`game_mode`=?, `statistics`=?, `is_flying`=?, `advancements`=?," +
                             "`location`=? WHERE `player_id`=(SELECT `id` FROM " + cluster.playerTableName() +
                             " WHERE `uuid`=?);")
        ) {
    ...
    private void insertPlayerData(PlayerData playerData, Settings.SynchronisationCluster cluster) {
        try (Connection connection = getConnection(cluster.clusterId());
             PreparedStatement statement = connection.prepareStatement(
                     "INSERT INTO " + cluster.dataTableName() + " (" +
                             "`player_id`,`version_uuid`,`timestamp`,`inventory`," +
                             "`ender_chest`,`health`,`max_health`,`health_scale`,`hunger`," +
                             "`saturation`,`saturation_exhaustion`,`selected_slot`,`status_effects`," +
                             "`total_experience`,`exp_level`,`exp_progress`,`game_mode`,`statistics`," +
                             "`is_flying`,`advancements`,`location`" +
                             ") VALUES((SELECT `id` FROM " + cluster.playerTableName() +
                             " WHERE `uuid`=?),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);")
        ) {
